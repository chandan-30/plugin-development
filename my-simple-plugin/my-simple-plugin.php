...
/**
 ...
 function msp_fetch_rows($post_id) {
    ...
    //check if transient exists with the key, if yes return
    if( $get_tran = get_transient('msp_transient' . $post_id) ) {
        return $get_tran;
    }
    $res = $wpdb->get_row("SELECT * from $table_name WHERE post_id = ". intval( $post_id ) );
    //set the transient value at first
    set_transient('msp_transient' . $post_id, $res);
    return res;
 }
/**
